...
struct WalArgs {
    ...
    #[arg(
        short = 's',
        long = "skip-terminal",
        help = "Skip setting terminal colors"
    )]
    skip_terminal: bool,
    ...
    #[arg(
        short = 'n',
        long = "skip-wallpaper",
        help = "Skip wallpaper-related operations"
    )]
    skip_wallpaper: bool,
    ...
    #[arg(
        ...
        help = "Force regeneration, overwrite cache"
    )]
    ...
    #[arg(
        short = 'o',
        long = "other",
        help = "Run custom script after generation"
    )]
    other: Option<String>,
...
async fn set_wallpaper(args: SetArgs) -> Result<()> {
    ...
    let wallpaper_path = if args.reload {
        ...
        match args.source {
            Source::Picsum => fetch_picsum(&output_dir, &resolution_str).await?,
            Source::Unsplash => {
